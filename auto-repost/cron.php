...
function addCronTask()
{
    ...
    foreach ($Schedules->getDataAs($as) as $sc) {
        ...
        if($aval){
            $comment = namespace\_get_comment($comments, $do_spintax);
        }
        ...
        if ($target->type == "hashtag") {
            ...
            try {
                ...
                $maxId = NULL;
                do {
                    $feed = $Instagram->hashtag->getFeed($hashtag,$rank_token,$maxId);
                    $items= array_merge($items, $feed->getItems());
                    $ranked = $feed->getRankedItems();
                    if (is_array($ranked))
                    {
                        $items = array_merge($items, $ranked);
                    }
            ...
            try {
                ...
                $maxId = NULL;
                do {
                    $feed = $Instagram->hashtag->getFeed($target->id,$rank_token,$maxId);
                    $items= array_merge($items, $feed->getItems());
                    $ranked = $feed->getRankedItems();
                    if (is_array($ranked))
                    {
        ...
        foreach ($items as $item) {
            ...
            try {
                if(!namespace\filterResults($sc,$item)){
                    continue;
                }
            } catch (\Exception $e) {
                echo  $e->getMessage();
        ...
        try {
            ...
            if($aval){
                $com = $res->getMedia()->getId();
                $com2 = $Instagram->media->comment($com, $comment);
            }
        } catch (\Exception $e) {
...
function _get_comment($comments, $do_spintax = false)
{
...
function filterResults($sc,$target)
{

    if ((bool)$sc->get("data.filter_picture"))
    {
        //$filter_count++;
        if ($target->getUser()->getProfilePicUrl() == "")
        {
            return false;
        }
    }
    ...
    $filter_comment_min = (int)$sc->get("data.filter_comment_min");
    $filter_comment_max = (int)$sc->get("data.filter_comment_max");
    $filter_like_min = (int)$sc->get("data.filter_like_min");
    $filter_like_max = (int)$sc->get("data.filter_like_max");
    if ($filter_comment_min > 0)
    {
        if ($filter_comment_min > $commentcount){
            return false;
        }
    }
    if ($filter_comment_max > 0)
    {
        if ($filter_comment_max > $commentcount){
            return false;
        }
    }
    if ($filter_like_min > 0)
    {
        if ($filter_like_min > $likecount){
            return false;
        }
    }
    if ($filter_like_max > 0)
    {
        if ($filter_like_max > $commentcount){
            return false;
        }
    }
    return true;
